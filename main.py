...
st.title("🇮🇩 Dashboard Ketenagakerjaan (2018-2023)")
...
if not df_makro.empty and not df_lapangan.empty:
    latest_year = 2023
    tpt_now = df_makro[(df_makro['Tahun'] == latest_year) & (df_makro['Bulan'] == 'Agustus') & (df_makro['Status'] == 'Pengangguran')]['Persentase'].values[0]
    tpt_prev = df_makro[(df_makro['Tahun'] == latest_year-1) & (df_makro['Bulan'] == 'Agustus') & (df_makro['Status'] == 'Pengangguran')]['Persentase'].values[0]
    gaji_now = df_lapangan[df_lapangan['Tahun'] == latest_year]['Total'].mean()
    
    col_kpi1, col_kpi2, col_kpi3, col_kpi4 = st.columns(4)
    col_kpi1.metric("TPT (Ags 2023)", f"{tpt_now}%", f"{round(tpt_now - tpt_prev, 2)}%", delta_color="inverse")
    col_kpi2.metric("Rata-rata Upah", f"Rp {gaji_now/1000:,.0f}k", "Ribuan per bulan") # Format ribu agar pendek
    col_kpi3.metric("Fokus Data", "Nasional", "34 Provinsi")
    col_kpi4.metric("Status Data", "Cleaned", "Ready")


CHART_HEIGHT = 240 
MARGIN_CONF = dict(l=10, r=10, t=30, b=10)

c1, c2 = st.columns(2)

with c1:
    st.markdown("### 📈 Tren Pengangguran")
    if not df_makro.empty:
        ...
        fig = px.line(df_tpt, x='Waktu', y='Persentase', markers=True, template='plotly_white')
        fig.update_traces(line_color='#FF4B4B', line_width=2)
        fig.update_layout(height=CHART_HEIGHT, margin=MARGIN_CONF, xaxis_title=None, yaxis_title=None)
        st.plotly_chart(fig, use_container_width=True, key="c1")

with c2:
    st.markdown("### 👥 Siapa yang Menganggur? (2023)")
    if not df_umur.empty:
        df_filt = df_umur[(df_umur['Tahun_Real'] == 2023) & (df_umur['Kelompok Umur'] != 'Rata-Rata')]
        fig = px.bar(df_filt, x='Kelompok Umur', y='TPT', color='TPT', color_continuous_scale='Reds')
        fig.update_layout(height=CHART_HEIGHT, margin=MARGIN_CONF, xaxis_title=None, coloraxis_showscale=False)
        st.plotly_chart(fig, use_container_width=True, key="c2")

c3, c4 = st.columns(2)

with c3:
    st.markdown("### 💰 Top Upah Sektor Industri (2023)")
    if not df_lapangan.empty:
        df_map = df_lapangan[df_lapangan['Tahun'] == 2023].sort_values('Industri', ascending=False).head(7)
        fig = px.bar(df_map, x='Industri', y='Provinsi', orientation='h', color='Industri', color_continuous_scale='Viridis')
        fig.update_layout(height=CHART_HEIGHT, margin=MARGIN_CONF, yaxis=dict(autorange="reversed"), xaxis_title=None, coloraxis_showscale=False)
        st.plotly_chart(fig, use_container_width=True, key="c3")

with c4:
    st.markdown("### 🎓 Gap Upah: SD vs SMA (2023)")
    if not df_pendidikan.empty:
        df_edu = df_pendidikan[df_pendidikan['Tahun'] == 2023].copy()
        df_edu['Gap'] = df_edu['SMA_Keatas'] - df_edu['SD']
        df_edu = df_edu.sort_values('Gap').tail(7)

        fig = go.Figure()
        fig.add_trace(go.Scatter(x=df_edu['SD'], y=df_edu['Provinsi'], mode='markers', name='SD', marker=dict(color='red', size=8)))
        fig.add_trace(go.Scatter(x=df_edu['SMA_Keatas'], y=df_edu['Provinsi'], mode='markers', name='SMA+', marker=dict(color='blue', size=8)))
        
        for i in range(len(df_edu)):
            fig.add_shape(type='line', x0=df_edu.iloc[i]['SD'], y0=df_edu.iloc[i]['Provinsi'],
                          x1=df_edu.iloc[i]['SMA_Keatas'], y1=df_edu.iloc[i]['Provinsi'],
                          line=dict(color='gray', width=1))
        
        fig.update_layout(height=CHART_HEIGHT, margin=MARGIN_CONF, showlegend=False, xaxis_title=None)
...
with tabs[0]:
    if not df_lapangan.empty:
        ...
        fig.update_layout(title="Evolusi Kesenjangan Upah (Tertinggi vs Terendah)", 
                          yaxis2=dict(title="Gap", overlaying='y', side='right'), legend=dict(orientation="h"))
        st.plotly_chart(fig, use_container_width=True, key="deep_chart_disp")

with tabs[1]:
    if not df_umur.empty:
        df_gen = df_umur[df_umur['Kelompok Umur'] != 'Rata-Rata']
        fig = px.line(df_gen, x='Tahun_Real', y='TPT', color='Kelompok Umur', markers=True, title="Dampak Pandemi per Generasi")
        fig.add_vrect(x0=2019.5, x1=2020.5, fillcolor="red", opacity=0.1, line_width=0)
        st.plotly_chart(fig, use_container_width=True, key="deep_chart_gen")

with tabs[2]:
    if not df_lapangan.empty:
        t_dom = st.slider("Tahun:", 2018, 2023, 2023, key='deep_slider_dom')
        df_scat = df_lapangan[df_lapangan['Tahun'] == t_dom].dropna(subset=['Pertanian', 'Industri', 'Total'])
        if not df_scat.empty:
            fig = px.scatter(df_scat, x='Pertanian', y='Industri', hover_name='Provinsi', size='Total', color='Total',
                             title=f"Kuadran Ekonomi ({t_dom})")
            fig.add_shape(type="line", x0=0, y0=0, x1=4000000, y1=4000000, line=dict(color="grey", dash="dash"))
            st.plotly_chart(fig, use_container_width=True, key="deep_chart_dom")

with tabs[3]:
    if not df_makro.empty:
        df_seas = df_makro[df_makro['Status'] == 'Pengangguran']
        fig = px.bar(df_seas, x='Tahun', y='Persentase', color='Bulan', barmode='group', title="Pola Musiman: Feb vs Ags")
        st.plotly_chart(fig, use_container_width=True, key="deep_chart_seas")